...
LRESULT CALLBACK simWndProc(HWND hwnd, UINT iMsg, WPARAM wParam,
  ...
  case WM_MOUSEMOVE:
    ...
    //     processMouseXY( pt.x, pt.y, (s16) HIWORD(wParam) / 120,
    //     LOWORD(wParam), 0);
    //   }
    //   return 0;
  case WM_LBUTTONDOWN:
...
void bx_win32_gui_c::graphics_frame_update(const u32 *pixels, unsigned width,
                                           unsigned height) {
  ...
  StretchDIBits(MemoryDC, 0, 0, (int)width, (int)height, 0, 0, (int)width,
                (int)height, pixels, (const BITMAPINFO *)&bih, DIB_RGB_COLORS,
                SRCCOPY);

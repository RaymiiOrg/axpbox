...
CDEC21143::CDEC21143(CConfigurator *confg, CSystem *c, int pcibus, int pcidev)
...
#ifdef _WIN32
  {
    ...
    f_pcap_dispatch =
        (int (*)(void *, int, pcap_handler callback, unsigned char *user))
            GetProcAddress(libhandle, "pcap_dispatch");
    f_pcap_create = (void *(*)(const char *, char *))GetProcAddress(
        libhandle, "pcap_create");
...
void CDEC21143::init() {
    ...
    if (i == 1)
      ...
      for (;;) {
        ...
        if (fgets(input_buf, sizeof(input_buf), stdin) == NULL)
          FAILURE(Runtime,
                  "Unexpected end of input while selecting network interface");
  ...
  if ((fp = (pcap_t *)pcap_open(
           cfg, 65536 /*snaplen: capture entire packets */,
  ...
  if ((fp = pcap_open_live(cfg, 65536 /*snaplen: capture entire packets */,
                           1 /*promiscuous */,
                           10 /*packet buffer timeout: [ms] */, errbuf)) ==
      NULL) // connect to pcap...
#endif
...
void CDEC21143::SetupFilter() {
  ...
  numUnique = 0;
  for (i = 0; i < 16; i++) {
    if ((mac[i][0] | mac[i][1] | mac[i][2] | mac[i][3] | mac[i][4] |
         mac[i][5]) == 0)
      continue;
  ...
  if (!(promisc || receive_all)) {
    ...
    if (numUnique == 0) {
      ...
      char self[20];
      sprintf(self, "%02x:%02x:%02x:%02x:%02x:%02x", state.mac[0], state.mac[1],
              state.mac[2], state.mac[3], state.mac[4], state.mac[5]);
  ...
  } /* else PR/RA: leave filter as match-all broadcasts + everything (no extra
       terms) */

#if defined(DEBUG_NIC_FILTER)
  printf("FILTER = %s.   \n", filter);
#endif
  /* In promiscuous/receive-all we still compile an empty/very permissive
   * filter. */
  if (promisc || receive_all) {
